...
// just that `out` came out right. Checked across THREE separate trigger
...
module race_value_tb;
  ...
  initial begin
    ...
    for (i = 0; i < 3; i = i + 1) begin
      ...
      $display("trial %0d: out=%0d done_ha=%0d done_hb=%0d",
                i, out, dut.__done_pick_ha, dut.__done_pick_hb);
      if (out !== 8'd11) begin
        $display("FAIL: expected out == 11 (A's result), got %0d", out);
        failed = 1;
      end
      if (dut.__done_pick_hb !== 1'b0) begin
        $display("FAIL: expected B to never actually complete (scheduler priority should have blocked it) -- if this fires, out's correctness above no longer proves what this test claims");
        failed = 1;
      end
    end
