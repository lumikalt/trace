// Drives examples/conflict_free_check.tr through real module ports.
//
// Proves the compiler-inserted `conflict_free` assertion is a genuine
// runtime check, not dead code that always passes: a cycle where the
// claim actually holds (`we_a`/`we_b` never both 1) produces no
// assertion failure, while a cycle that deliberately violates the
